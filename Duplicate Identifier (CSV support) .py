...
class DuplicateDataIdentifierApp:
    ...
    def select_file(self, file_type):
        file_selected = filedialog.askopenfilename(filetypes=[("Excel/CSV Files", "*.xlsx *.csv")])
    ...
    def process_single_file(self):
        ...
        try:
            if self.primary_file_path.get().lower().endswith('.csv'):
                df_primary = pd.read_csv(self.primary_file_path.get(), header=self.header_row.get() - 1)
            else:
                df_primary = pd.read_excel(self.primary_file_path.get(), header=self.header_row.get() - 1)

            if self.supporting_file_path.get().lower().endswith('.csv'):
                df_supporting = pd.read_csv(self.supporting_file_path.get(), header=self.header_row.get() - 1)
            else:
...
def open_output_folder(self):
    ...
    try:
        ...
        messagebox.showwarning("Note", f"The folder cannot be opened automatically: {str(e)}")
